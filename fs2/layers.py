...
class ConformerEncoderLayer(nn.TransformerEncoderLayer):
...
class VarianceAdaptor(nn.Module):
    ...
    def forward(
        ...
        if inference and self.variance_gan:
            x_true, tgt_mask_true = self.length_regulator(
                x, targets["duration"], self.max_length
            )
            ...
            if self.variance_levels[i] == "frame":
                ...
                if self.variance_gan:
                    if not inference:
                        ...
                        out_true = self.encoders[var](
                            x_true, targets[f"variances_{var}"], tgt_mask_true
                        )[1]
                        out_fake = out
                    result[f"variances_{var}_disc_fake"] = self.variance_discriminators[
                        var
