...
class FastSpeech2(pl.LightningModule):
    def __init__(
        ...
        self.encoder = nn.TransformerEncoder(
            ConformerEncoderLayer(
                self.config.model.encoder.hidden_dim,
                self.config.model.encoder.heads,
                conv_in=self.config.model.encoder.hidden_dim,
                conv_filter_size=self.config.model.conv_filter_size,
                conv_kernel=(
                    self.config.model.encoder.conv_kernel_sizes[0],
                    1,
                ),
                batch_first=True,
                dropout=self.config.model.encoder.dropout,
                conv_depthwise=self.config.model.encoder.depthwise,
            ),
        ...
        self.decoder = nn.TransformerEncoder(
            ConformerEncoderLayer(
                ...
                self.config.model.decoder.heads,
                conv_in=self.config.model.decoder.hidden_dim,
                conv_filter_size=self.config["model"]["transformer"][
        ...
        if self.config["model"]["multilingual"]:
            self.lang_embedding = nn.Embedding(
                len(self.lang2id), self.config.model.decoder.hidden_dim
            )
        if (
