...
class ConformerConfig(ConfigModel):
    layers: int = Field(default=4, description="The number of layers in the Conformer.")
    heads: int = Field(
        default=2,
    ...
    feedforward_dim: int = Field(
        default=1024, description="The number of dimensions in the feedforward layers."
    )
    conv_kernel_size: int = Field(
        default=9,
        description="The size of the kernel in each convoluational layer of the Conformer.",
    )
    dropout: float = Field(default=0.2, description="The amount of dropout to apply.")
...
class VariancePredictorBase(ConfigModel):
    loss: VarianceLossEnum = Field(
        default=VarianceLossEnum.mse,
        description="The loss function to use when calculate variance loss. Either 'mse' or 'mae'.",
    )
    n_layers: int = Field(
        default=5, description="The number of layers in the variance predictor module."
    )
    kernel_size: int = Field(
        default=3,
        description="The kernel size of each convolutional layer in the variance predictor module.",
    )
    dropout: float = Field(default=0.5, description="The amount of dropout to apply.")
    input_dim: int = Field(
        default=256,
    ...
    depthwise: bool = Field(
        default=True, description="Whether to use depthwise separable convolutions."
    )
...
class VariancePredictorConfig(VariancePredictorBase):
    level: VarianceLevelEnum = Field(
        default=VarianceLevelEnum.phone,
...
class FastSpeech2ModelConfig(ConfigModel):
    ...
    use_postnet: bool = Field(
        default=True, description="Whether to use a postnet module."
    )
...
class FastSpeech2TrainingConfig(BaseTrainingConfig):
    ...
    postnet_loss_weight: float = Field(
        default=1.0, description="Multiply the postnet loss by this weight"
    )
    pitch_loss_weight: float = Field(
        default=0.1, description="Multiply the pitch loss by this weight"
    )
    energy_loss_weight: float = Field(
        default=0.1, description="Multiply the energy loss by this weight"
    )
    duration_loss_weight: float = Field(
        default=0.1, description="Multiply the duration loss by this weight"
    )
    attn_ctc_loss_weight: float = Field(
        default=0.1, description="Multiply the Attention CTC loss by this weight"
    )
...
class FastSpeech2Config(BaseModelWithContact):
    ...
    path_to_text_config_file: Optional[FilePath] = Field(
        default=None, description="The path of a text configuration file."
    )
